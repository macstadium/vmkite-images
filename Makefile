...
macos-10.12:
	time env PACKER_LOG=$(packer_log) packer build $(packer_args) \
		...
		-var output_directory="$(output_directory)/macos-10.12.3" \
		-var version=10.12.3 \
		macos.json
...
macos-10.11:
	time env PACKER_LOG=$(packer_log) packer build $(packer_args) \
		...
		-var iso_checksum_type=sha256 \
		-var output_directory="$(output_directory)/macos-10.11.6" \
		-var version=10.11.6 \
		macos.json
...
macos-10.10:
	time env PACKER_LOG=$(packer_log) packer build $(packer_args) \
		...
		-var iso_checksum_type=sha256 \
		-var output_directory="$(output_directory)/macos-10.10.5" \
		-var version=10.10.5 \
		macos.json
...
ubuntu-16.04:
	time env PACKER_LOG=$(packer_log) packer build $(packer_args) \
		-var headless=$(headless) \
		-var output_directory="$(output_directory)/ubuntu-16.04.2" \
